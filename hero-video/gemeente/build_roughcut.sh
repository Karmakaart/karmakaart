#!/usr/bin/env bash
# KarmaKaart gemeente-video — ROUGH CUT / animatic (ffmpeg), v2 met "wijk die oplicht" dots.
...
# Tussenin de 5 character-clips. Silent (VO-bed). Gepolijste dots/camera = nog AE-optie.
set -euo pipefail

HERE="$(cd "$(dirname "$0")" && pwd)"
CL="$HERE/clips"; KF="$HERE/keyframes"; MAP="$KF/map_wijk_base_v1.png"
B="$HERE/_build"; mkdir -p "$B"
OUT="$HERE/gemeente_roughcut_v2.mp4"
FONT="$HERE/_font.ttf"
W=1280; H=720; FPS=24
...
O3=$(awk "BEGIN{print 6+4+5-3*$XF}")
O4=$(awk "BEGIN{print 6+4+5+3-4*$XF}")
O5=$(awk "BEGIN{print 6+4+5+3+3-5*$XF}")
O6=$(awk "BEGIN{print 6+4+5+3+3+3-6*$XF}")
O7=$(awk "BEGIN{print 6+4+5+3+3+3+6-7*$XF}")
