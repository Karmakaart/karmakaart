...
Eindmontage in **After Effects**. ~43s, 1280×720 (desktop) + 9:16-crop (mobiel). Stijl: KarmaKaart painterly (`fp-1`), NIET SlowCarb. Voice-over: jouw stem (volledige tekst in `design.md` §6).
...
| A | 11–14s | `clips/shot2b_daan_arrival_anim_v1.mp4` (keyframe: `keyframes/shot2b_daan_arrival_v1.png`) | Daan komt **gesloten** aan bij Henks voordeur (gereserveerde houding, loopt de stoep op, aarzeling bij de deur). Bridge straat→interieur. |
| 3 | 14–26s | `clips/shot3_daan_henk_koffie_v2.mp4` | De koffie-beat (warm; Henk vertelt, Daan luistert). Tijdens/na deze shot: laat op de kaart-laag **dot 1 warm oplichten** (grijs→goud). |
| B | 26–29s | `clips/shot3b_daan_leaves_anim_v1.mp4` (keyframe: `keyframes/shot3b_daan_leaves_v1.png`) | Daan loopt **lichter/opgeveerd** weg de straat in (hoofd omhoog, open). Op de kaart licht nu **Daans eigen dot** warm op — label-loos, gewoon een dot zoals de andere ("Daan zelf → een stap vooruit"). Het weglopen-de-diepte-in leidt de pullback (beat 5) in. |
| 4 | 29–35s | `clips/cascade_jan_koffie_anim_v1.mp4`, `clips/cascade_amir_taalmaatje_anim_v2.mp4`, `clips/cascade_marcus_voorlezen_anim_v1.mp4` | Snelle cascade: elke clip kort in beeld (geanimeerd); op de kaart springen meer dots warm aan. (Bron-stills in `cascade/`.) |
| 5 | 35–40s | `keyframes/map_wijk_base_v1.png` (vol kleur, alle dots warm) | Uittrekken: de hele wijk **gloeit**, verbonden. Grijs→warm volledig omgeslagen. |
| 6 | 40–43s | KarmaKaart-logo | Eindkaart + regel: *"KarmaKaart. Eén kaart. De hele wijk in beweging."* |
...
- `keyframes/shot2b_daan_arrival_v1.png` — beat A keyframe (Daan aankomst, gesloten)
- `keyframes/shot3_daan_henk_koffie_v1.png` — beat 3 keyframe
- `keyframes/shot3b_daan_leaves_v1.png` — beat B keyframe (Daan afscheid, opgeveerd)
- `clips/shot2_daan_phone_anim_v1.mp4` — beat 2 animatie (tik + knik)
- `clips/shot2b_daan_arrival_anim_v1.mp4` — beat A animatie (loopt de stoep op, aarzelt bij de deur)
- `clips/shot3_daan_henk_koffie_v2.mp4` — beat 3 animatie (**gebruik v2** — warme verteller, geen mondvervorming)
- `clips/shot3b_daan_leaves_anim_v1.mp4` — beat B animatie (loopt opgeveerd weg, de diepte in)
...
7 bewegende character-clips totaal (opening + aankomst + koffie + afscheid + 3 cascade) + de AE-kaart.
...
*(De Daan-boog beats A & B zijn nog niet in v2 verwerkt — zie `gemeente_roughcut_v3` zodra gebouwd.)*
...
- Optioneel: een langere koffie-clip (10s); 9:16-crop voor mobiel.
