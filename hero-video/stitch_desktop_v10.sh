#!/usr/bin/env bash
# KarmaKaart desktop hero v10 — tightened final-ish cut to hand off to After Effects.
# Same 7 clips + same good START points as v9, but DURATIONS trimmed of slack so
# each beat lands in ~3-5s. ~26s total. AE does the real final timing/grade.

set -euo pipefail

HERE="$(cd "$(dirname "$0")" && pwd)"
CLIPS="$HERE/clips"
OUT="$HERE/karmakaart_hero_desktop_v10.mp4"
...
# Keep v9 START points (tuned to catch the expressive beat); trim DURATIONS.
S1=2.0; D1=5.0   # Maya   2.0-7.0  trigger: frustration -> determined -> tap -> resolve
S2=1.5; D2=5.0   # Marcus 1.5-6.5  look at phone -> smile -> stand -> walk
S3=1.0; D3=4.5   # Lisa   1.0-5.5  stand still -> look -> expression -> turn
S4=0.5; D4=3.5   # Jan    0.5-4.0  push up from kneeling
S5=1.0; D5=4.5   # connection 1.0-5.5  drop -> look at each other -> shared smile
S6=0.3; D6=2.7   # aerial 0.3-3.0
S7=0.0; D7=4.0   # finale logo
XF=0.5
