#!/usr/bin/env bash
# Stitch KarmaKaart desktop hero v2 — uses latest approved per-shot clips.
# Pacing matches original (Maya 4.0s, Marcus/Lisa/Jan 2.5s each, Group 4.0s,
# Birdseye 3.0s, Finale 5.0s) with 0.5s xfade + acrossfade between clips.

set -euo pipefail

HERE="$(cd "$(dirname "$0")" && pwd)"
CLIPS="$HERE/clips"
OUT="$HERE/karmakaart_hero_desktop_v2.mp4"

C1="$CLIPS/shot1_maya_v3.mp4"
C2="$CLIPS/shot2a_marcus_v2.mp4"
C3="$CLIPS/shot2b_lisa_v2.mp4"
C4="$CLIPS/shot2c_jan_v2.mp4"
C5="$CLIPS/shot3_group_v4_seedance.mp4"
C6="$CLIPS/shot4_birdseye_v2.mp4"
C7="$CLIPS/shot5_horizon_logo_v2.mp4"

# Per-clip target durations (seconds, after trim)
D1=4.0; D2=2.5; D3=2.5; D4=2.5; D5=4.0; D6=3.0; D7=5.0
XF=0.5

# Cumulative xfade offsets (start of transition on combined timeline)
...
O5=$(awk "BEGIN{print $D1 + $D2 + $D3 + $D4 + $D5 - 5*$XF}") # 13.0
O6=$(awk "BEGIN{print $D1 + $D2 + $D3 + $D4 + $D5 + $D6 - 6*$XF}") # 15.5
